...
protected:
    ...
    try { if( pkey.empty() ){ return false; }
        ...
        if( hash != sha.get() )                  { return false; } return true;

    } catch( ... ){ return false; }}

    bool verify( const string_t& path ) const noexcept { try {
        file_t file ( path, "r" );
...
public:
    ...
    void free() const noexcept { if(obj->state==0){ return; } obj->state=0; onClose.emit(); }
    ...
    void write_private_key( const string_t& path, const string_t& pass=nullptr ) const noexcept {
    ...
    string_t write_private_key_to_memory( const string_t& pass=nullptr ) const noexcept {
        ...
        { "size", obj->size }, { "type", "PRIVATE" } })), ctx.mask ));
    ...
    void write_public_key( const string_t& path ) const noexcept {
    ...
    string_t write_public_key_to_memory() const noexcept {
        ...
        { "name", obj->name }, { "mail", obj->mail }, { "comment", obj->cmmt },
        { "expiration", array_t<uint>({ obj->stmp[0], obj->stmp[1] }) },
        { "size", obj->size }, { "type", "PUBLIC" } })), ctx.mask ));
    ...
    try {
        ...
        auto dec = crypto::decrypt::AES_256_ECB( sec );
        auto XOR = crypto::decrypt::XOR( ctx.mask );
